document.addEventListener("DOMContentLoaded", () => {
    ...
    posters.forEach(poster => {
        ...
        poster.addEventListener('mousemove', (e) => {
            ...
            const x = e.clientX - rect.left; // 포스터 내부의 X 위치
            const y = e.clientY - rect.top; // 포스터 내부의 Y 위치

            // 회전 효과
            const rotateY = ((x / rect.width) - 0.5) * 80; // -10° ~ 10°
            const rotateX = ((y / rect.height) - 0.5) * -80; // -10° ~ 10°
            poster.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg)`;
        });

        poster.addEventListener('mouseleave', () => {
            // 마우스가 떠나면 초기 상태로 복구
            poster.style.transition = "transform 0.5s ease"; // 부드러운 복귀
        ...
        poster.addEventListener('click', () => {
            if (overlay.style.opacity === "1") {
                overlay.style.opacity = "0"; // 필름 효과 비활성화
                overlay.style.transition = "opacity 0.3s ease-in-out";
            } else {
                overlay.style.transition = "opacity 0.3s ease-in-out";
                overlay.style.opacity = "1"; // 필름 효과 활성화
            }
        });
...
function handleScroll() {
  // 화면에서 200px 이상 스크롤되었을 때, 요소를 천천히 내려옴
